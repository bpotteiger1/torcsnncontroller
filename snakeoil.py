...
def drive_example(c):
    ...
    # Write training data
    if (stepcount%100000==0 and stepcount<=1000000):
        print(stepcount,'...')
    if (stepcount<1000000):
        row=[S['angle'],S['trackPos'],S['speedX'],R['gear'],S['track'][7],S['track'][8],S['track'][9],S['track'][10],S['track'][11],R['accel'],R['brake'],R['steer']]
        rows[stepcount]=row
#        rows.append(row)
    if (stepcount==1000000):
