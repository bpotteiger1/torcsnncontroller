...
def drive_example(c):
    ...
    target_speed=50
#    a=time.time()
    row=[float(S['angle']),float(S['trackPos']),float(S['speedX']),float(R['gear']),float(S['track'][7]),float(S['track'][8]),float(S['track'][9]),float(S['track'][10]),float(S['track'][11])]
    
    inputs=np.array(row)[np.newaxis]
    outputs=model.predict(inputs,batch_size=1)
    #print(outputs)
    steering=float(outputs[0][0])
    R['steer']=steering
...
#        R['accel']-= .01
#    if S['speedX']<10:
#           tempaccel=R['accel']+ (1/(S['speedX']+.1))
#           newaccel=min(1,tempaccel)
#           R['accel']=newaccel
